// ============================================================
// PROFIT 24 - COMPLETE BACKEND SERVER v11
// Multiple Plans, Multiple Withdrawals, Full Features
// ============================================================
...
// User - Multiple Active Plans (Array)
...
function generatePID(whatsapp) {
    var c = whatsapp.replace(/[\s\-\+\(\)]/g, '');
    return c.length >= 5 ? c.slice(-5) : c.padStart(5, '0');
}
...
function initBot() {
        ...
        return {
            reply_markup: {
                inline_keyboard: [
                    ...
                    [{ text: '🔄 Fund Transfer', callback_data: 'ftset' }, { text: '🏦 Accounts', callback_data: 'accs' }],
                    [{ text: '⚙️ Settings', callback_data: 'sett' }, { text: '🔗 Referral', callback_data: 'ref' }],
                    [{ text: '❓ FAQ', callback_data: 'faq' }, { text: '📢 Broadcast', callback_data: 'bcast' }],
    ...
    bot.onText(/\/start|\/admin/, function(msg) {
        ...
        bot.sendMessage(adminId, '🔐 *ADMIN PANEL*\n\nAll controls via buttons:', { parse_mode: 'Markdown', ...mainMenu() });
    });
    ...
    bot.on('callback_query', async function(q) {
        ...
        try {
            if (a === 'dash') {
                ...
                users.forEach(function(u, i) { m += (i + 1) + '. ' + (u.status === 'blocked' ? '🔴' : '🟢') + ' ' + u.username + ' | PID:' + u.pid + ' | Plans:' + (u.activePlans?.length || 0) + ' | PKR ' + (u.balance || 0).toFixed(2) + '\n'; });
            ...
            } else if (a === 'addp') { botSessions[cid] = { s: 'addp' }; await bot.sendMessage(cid, '➕ Send: Name | Amount | Profit% | Days\nExample: Premium | 5000 | 12 | 60\n/cancel'); }
            else if (a === 'delp') { var plans = await Plan.find().sort({ planId: 1 }).lean(); var btns = plans.map(function(p) { return [{ text: '🗑️ ' + p.planId + '. ' + p.name, callback_data: 'delp_' + p.planId }]; }); btns.push([{ text: '🔙 Back', callback_data: 'plans' }]); await bot.editMessageText('Select to delete:', { chat_id: cid, message_id: q.message.message_id, reply_markup: { inline_keyboard: btns } }); }
            else if (a === 'togp') { var plans = await Plan.find().sort({ planId: 1 }).lean(); var btns = plans.map(function(p) { return [{ text: (p.isActive ? '✅' : '❌') + ' ' + p.planId + '. ' + p.name, callback_data: 'togp_' + p.planId }]; }); btns.push([{ text: '🔙 Back', callback_data: 'plans' }]); await bot.editMessageText('Select to toggle:', { chat_id: cid, message_id: q.message.message_id, reply_markup: { inline_keyboard: btns } }); }
            else if (a.startsWith('delp_')) { var pid = parseInt(a.split('_')[1]); await Plan.deleteOne({ planId: pid }); await bot.sendMessage(cid, '✅ Deleted!', mainMenu()); }
            else if (a.startsWith('togp_')) { var pid = parseInt(a.split('_')[1]); var p = await Plan.findOne({ planId: pid }); if (p) { p.isActive = !p.isActive; await p.save(); } await bot.sendMessage(cid, '✅ Toggled!', mainMenu()); }
            else if (a === 'ftset') { var fte = await Setting.findOne({ key: 'fundTransferEnabled' }), fee = await Setting.findOne({ key: 'fundTransferFee' }); var m = '🔄 *FUND TRANSFER*\n\nStatus: ' + (fte?.value !== false ? '✅' : '❌') + '\nFee: ' + (fee?.value || 0) + '%'; var kb = { reply_markup: { inline_keyboard: [[{ text: (fte?.value !== false ? '❌ Disable' : '✅ Enable'), callback_data: 'tftoggle' }], [{ text: '💰 Set Fee', callback_data: 'ftfee' }], [{ text: '🔙 Menu', callback_data: 'dash' }]] } }; await bot.editMessageText(m, { chat_id: cid, message_id: q.message.message_id, parse_mode: 'Markdown', ...kb }); }
            else if (a === 'tftoggle') { var cur = await Setting.findOne({ key: 'fundTransferEnabled' }); var nv = cur?.value !== false ? false : true; await Setting.findOneAndUpdate({ key: 'fundTransferEnabled' }, { value: nv }, { upsert: true }); await bot.sendMessage(cid, '✅ TF ' + (nv ? 'Enabled' : 'Disabled'), mainMenu()); }
            else if (a === 'ftfee') { botSessions[cid] = { s: 'ftfee' }; await bot.sendMessage(cid, '💰 Send fee % (0-100):\n/cancel'); }
            else if (a === 'accs') { var ep = await Setting.findOne({ key: 'easypaisaNumber' }), jc = await Setting.findOne({ key: 'jazzcashNumber' }); var m = '🏦 *ACCOUNTS*\n\nEP: ' + (ep?.value || 'N/A') + '\nJC: ' + (jc?.value || 'N/A'); var kb = { reply_markup: { inline_keyboard: [[{ text: '📱 Update EP', callback_data: 'setep' }], [{ text: '💼 Update JC', callback_data: 'setjc' }], [{ text: '🔙 Menu', callback_data: 'dash' }]] } }; await bot.editMessageText(m, { chat_id: cid, message_id: q.message.message_id, parse_mode: 'Markdown', ...kb }); }
            else if (a === 'setep') { botSessions[cid] = { s: 'setep' }; await bot.sendMessage(cid, '📱 Send: Number | Title\n/cancel'); }
            else if (a === 'setjc') { botSessions[cid] = { s: 'setjc' }; await bot.sendMessage(cid, '💼 Send: Number | Title\n/cancel'); }
            else if (a === 'sett') {
                ...
                var kb = { reply_markup: { inline_keyboard: [[{ text: '📝 Min WD', callback_data: 'smin' }, { text: '📝 Max WD', callback_data: 'smax' }, { text: '📝 Daily', callback_data: 'sdaily' }], [{ text: (de?.value !== false ? '❌ Dep' : '✅ Dep'), callback_data: 'togdep' }, { text: (we?.value !== false ? '❌ WD' : '✅ WD'), callback_data: 'togwd' }], [{ text: '🔙 Menu', callback_data: 'dash' }]] } };
                await bot.editMessageText(m, { chat_id: cid, message_id: q.message.message_id, parse_mode: 'Markdown', ...kb });
            } else if (a === 'smin') { botSessions[cid] = { s: 'smin' }; await bot.sendMessage(cid, 'Send min WD amount:\n/cancel'); }
            else if (a === 'smax') { botSessions[cid] = { s: 'smax' }; await bot.sendMessage(cid, 'Send max WD amount:\n/cancel'); }
            else if (a === 'sdaily') { botSessions[cid] = { s: 'sdaily' }; await bot.sendMessage(cid, 'Send daily limit:\n/cancel'); }
            else if (a === 'togdep') { var cur = await Setting.findOne({ key: 'depositEnabled' }); var nv = cur?.value !== false ? false : true; await Setting.findOneAndUpdate({ key: 'depositEnabled' }, { value: nv }, { upsert: true }); await bot.sendMessage(cid, '✅ Deposits ' + (nv ? 'On' : 'Off'), mainMenu()); }
            else if (a === 'togwd') { var cur = await Setting.findOne({ key: 'withdrawEnabled' }); var nv = cur?.value !== false ? false : true; await Setting.findOneAndUpdate({ key: 'withdrawEnabled' }, { value: nv }, { upsert: true }); await bot.sendMessage(cid, '✅ Withdrawals ' + (nv ? 'On' : 'Off'), mainMenu()); }
            else if (a === 'ref') { var r = await Setting.findOne({ key: 'referralBonus' }); var m = '🔗 *REFERRAL*\n\nBonus: ' + (r?.value || 11) + '%'; var kb = { reply_markup: { inline_keyboard: [[{ text: '📝 Change', callback_data: 'sref' }], [{ text: '🔙 Menu', callback_data: 'dash' }]] } }; await bot.editMessageText(m, { chat_id: cid, message_id: q.message.message_id, parse_mode: 'Markdown', ...kb }); }
            else if (a === 'sref') { botSessions[cid] = { s: 'sref' }; await bot.sendMessage(cid, 'Send bonus %:\n/cancel'); }
            else if (a === 'faq') { var faqs = await FAQ.find().sort({ order: 1 }).lean(); var m = '❓ *FAQs (' + faqs.length + ')*\n\n'; if (!faqs.length) m += 'None\n'; else faqs.forEach(function(f, i) { m += (i + 1) + '. ' + f.question + '\n\n'; }); var kb = { reply_markup: { inline_keyboard: [[{ text: '➕ Add', callback_data: 'addfaq' }], [{ text: '🗑️ Clear', callback_data: 'clrfaq' }], [{ text: '🔙 Menu', callback_data: 'dash' }]] } }; await bot.editMessageText(m, { chat_id: cid, message_id: q.message.message_id, parse_mode: 'Markdown', ...kb }); }
            else if (a === 'addfaq') { botSessions[cid] = { s: 'addfaq' }; await bot.sendMessage(cid, 'Send: Q | A\n/cancel'); }
            else if (a === 'clrfaq') { await FAQ.deleteMany({}); await bot.sendMessage(cid, '✅ Cleared!', mainMenu()); }
            else if (a === 'bcast') { botSessions[cid] = { s: 'bcast' }; await bot.sendMessage(cid, '📢 Send message to broadcast:\n/cancel'); }
            else if (a === 'search') { botSessions[cid] = { s: 'search' }; await bot.sendMessage(cid, '🔍 Send username or PID:\n/cancel'); }
            // APPROVE/REJECT DEPOSIT
            else if (a.startsWith('ad_') || a.startsWith('rd_')) {
                var parts = a.split('_'), st = parts[0] === 'ad' ? 'approved' : 'rejected', tid = parts[1];
                var tx = await Transaction.findById(tid);
                if (!tx) { await bot.sendMessage(cid, '❌ Not found'); return; }
                if (tx.status !== 'pending') { await bot.sendMessage(cid, '⚠️ Already ' + tx.status); return; }
                ...
                if (tx.telegramMsgId) {
                    ...
                    try {
                        var cap = (st === 'approved' ? '✅ *DEPOSIT APPROVED!*' : '❌ *DEPOSIT REJECTED!*') + '\n👤 ' + tx.username + '\n💵 PKR ' + tx.amount.toLocaleString();
                        if (tx.screenshot && fs.existsSync(tx.screenshot)) { await bot.editMessageCaption(cap, { chat_id: cid, message_id: tx.telegramMsgId, parse_mode: 'Markdown' }); }
                        else { await bot.editMessageText(cap, { chat_id: cid, message_id: tx.telegramMsgId, parse_mode: 'Markdown' }); }
                    } catch (e) { }
                }
                if (st === 'approved') {
                    var u = await User.findOne({ username: tx.username }), p = await Plan.findOne({ planId: tx.planId });
                    if (u && p) {
                        var dailyProfitAmount = p.amount * (p.dailyProfit / 100);
                        u.activePlans.push({ planId: p.planId, name: p.name, amount: p.amount, dailyProfit: dailyProfitAmount, startDate: new Date(), endDate: new Date(Date.now() + p.duration * 86400000), profitDays: 0 });
                        u.totalInvested += p.amount;
                        var firstProfit = dailyProfitAmount;
                        u.balance += firstProfit; u.totalEarned += firstProfit;
                        var lastPlan = u.activePlans[u.activePlans.length - 1];
                        lastPlan.profitDays += 1;
                        await u.save();
                        await new Transaction({ userId: u._id, username: u.username, type: 'profit', amount: firstProfit, status: 'approved' }).save();
                        if (u.referredBy) {
                ...
                bot.sendMessage(cid, '✅ *DEPOSIT ' + st.toUpperCase() + '!*', mainMenu());
            }
            // APPROVE/REJECT WITHDRAWAL
            else if (a.startsWith('aw_') || a.startsWith('rw_')) {
                var parts = a.split('_'), st = parts[0] === 'aw' ? 'approved' : 'rejected', tid = parts[1];
                var tx = await Transaction.findById(tid);
                if (!tx) { await bot.sendMessage(cid, '❌ Not found'); return; }
                if (tx.status !== 'pending') { await bot.sendMessage(cid, '⚠️ Already ' + tx.status); return; }
                tx.status = st; tx.processedAt = new Date(); await tx.save();
                ...
                if (tx.telegramMsgId) {
                    try { await bot.editMessageReplyMarkup({ inline_keyboard: [] }, { chat_id: cid, message_id: tx.telegramMsgId }); } catch (e) { }
                    try {
                        var cap = (st === 'approved' ? '✅ *WITHDRAWAL APPROVED!*' : '❌ *WITHDRAWAL REJECTED!*') + '\n👤 ' + tx.username + '\n💵 PKR ' + tx.amount.toLocaleString();
                ...
                if (st === 'rejected') {
                    ...
                    if (u) {
                        ...
                        bot.sendMessage(cid, 'ℹ️ Refund in 1 hour for ' + u.username);
                        setTimeout(async function() {
                            try {
                                u.balance += tx.amount; await u.save();
                                ...
                                bot.sendMessage(cid, '✅ Refunded PKR ' + tx.amount.toLocaleString() + ' → ' + u.username);
                            } catch (e) { }
                        }, 3600000);
                    }
                }
                bot.sendMessage(cid, '✅ *WITHDRAWAL ' + st.toUpperCase() + '!*', mainMenu());
            }
            else if (a.startsWith('ub_')) { var un = a.split('_')[1]; var u = await User.findOne({ username: un }); if (u) { u.status = u.status === 'active' ? 'blocked' : 'active'; await u.save(); bot.sendMessage(cid, '✅ ' + u.username + ' → ' + u.status); } }
    ...
    bot.on('message', async function(msg) {
        ...
        var s = botSessions[cid];
        if (text === '/cancel') { delete botSessions[cid]; return bot.sendMessage(cid, '❌ Cancelled', mainMenu()); }
        if (s && s.s === 'addp') { var p = text.split('|').map(function(x) { return x.trim(); }); if (p.length < 4) return bot.sendMessage(cid, '❌ Name|Amt|%|Days'); var lp = await Plan.findOne().sort({ planId: -1 }); await new Plan({ planId: (lp?.planId || 0) + 1, name: p[0], amount: parseInt(p[1]), dailyProfit: parseFloat(p[2]), duration: parseInt(p[3]) }).save(); delete botSessions[cid]; return bot.sendMessage(cid, '✅ Added!', mainMenu()); }
        if (s && s.s === 'ftfee') { var v = parseFloat(text); if (isNaN(v) || v < 0 || v > 100) return bot.sendMessage(cid, '❌ 0-100'); await Setting.findOneAndUpdate({ key: 'fundTransferFee' }, { value: v }, { upsert: true }); delete botSessions[cid]; return bot.sendMessage(cid, '✅ Fee: ' + v + '%', mainMenu()); }
        ...
        if (s && s.s === 'smin') { await Setting.findOneAndUpdate({ key: 'minWithdraw' }, { value: parseInt(text) }, { upsert: true }); delete botSessions[cid]; return bot.sendMessage(cid, '✅ Min: ' + text, mainMenu()); }
        if (s && s.s === 'smax') { await Setting.findOneAndUpdate({ key: 'maxWithdraw' }, { value: parseInt(text) }, { upsert: true }); delete botSessions[cid]; return bot.sendMessage(cid, '✅ Max: ' + text, mainMenu()); }
        if (s && s.s === 'sdaily') { await Setting.findOneAndUpdate({ key: 'maxDailyWithdraw' }, { value: parseInt(text) }, { upsert: true }); delete botSessions[cid]; return bot.sendMessage(cid, '✅ Daily: ' + text, mainMenu()); }
        if (s && s.s === 'sref') { await Setting.findOneAndUpdate({ key: 'referralBonus' }, { value: parseFloat(text) }, { upsert: true }); delete botSessions[cid]; return bot.sendMessage(cid, '✅ Bonus: ' + text + '%', mainMenu()); }
        if (s && s.s === 'addfaq') { var p = text.split('|').map(function(x) { return x.trim(); }); if (p.length < 2) return bot.sendMessage(cid, '❌ Q|A'); await new FAQ({ question: p[0], answer: p[1], order: await FAQ.countDocuments() + 1 }).save(); delete botSessions[cid]; return bot.sendMessage(cid, '✅ Added!', mainMenu()); }
        if (s && s.s === 'bcast') { var users = await User.find({ status: 'active' }).lean(); var cnt = 0; for (var i = 0; i < users.length; i++) { try { await bot.sendMessage(users[i]._id, '📢 *Admin*\n\n' + text, { parse_mode: 'Markdown' }); cnt++; } catch (e) { } } delete botSessions[cid]; return bot.sendMessage(cid, '✅ ' + cnt + '/' + users.length, mainMenu()); }
        if (s && s.s === 'search') {
            var u = await User.findOne({ $or: [{ username: text }, { pid: text }] }).lean();
            if (!u) { delete botSessions[cid]; return bot.sendMessage(cid, '❌ Not found', mainMenu()); }
            delete botSessions[cid];
            var plansInfo = '';
            if (u.activePlans && u.activePlans.length > 0) {
                u.activePlans.forEach(function(p) { plansInfo += '📋 ' + p.name + ' (D' + p.profitDays + '/60), '; });
                plansInfo = plansInfo.slice(0, -2);
            } else { plansInfo = 'None'; }
...
async function showPending(type, cid, msgId) {
    ...
    for (var i = 0; i < items.length; i++) {
        ...
        if (type === 'deposit' && it.screenshot && fs.existsSync(it.screenshot)) {
            try { sent = await bot.sendPhoto(cid, it.screenshot, { caption: m, parse_mode: 'Markdown', ...kb }); } catch (e) { sent = await bot.sendMessage(cid, m, { parse_mode: 'Markdown', ...kb }); }
...
async function notifyAdminTransfer(sender, sPID, receiver, rPID, amount, fee) {
    await notifyAdmin('🔄 <b>FUND TRANSFER</b>\n\n📤 ' + sender + ' (PID:' + sPID + ')\n📥 ' + receiver + ' (PID:' + rPID + ')\n💵 PKR ' + amount.toLocaleString() + '\n💰 Fee: PKR ' + fee.toFixed(2));
}
...
app.get('/api/dashboard', auth, async function(req, res) {
    ...
    res.json({
        success: true, username: user.username, pid: user.pid, profilePic: user.profilePic,
        balance: user.balance || 0, activePlans: user.activePlans || [],
        ...
        pendingDeposits: pendDep, pendingWithdrawals: pendWd
    });
});
...
// Deposit (Multiple Plans Allowed)
...
// Withdraw (Multiple Withdrawals Allowed)
...
// URDU + ENGLISH Chatbot
app.post('/api/chat', async function(req, res) {
    var msg = (req.body.message || '').trim();
    var isUrdu = /[\u0600-\u06FF]/.test(msg);
    var reply = '';
    if (isUrdu) {
        if (msg.includes('سرمایہ') || msg.includes('انویسٹ') || msg.includes('پلان')) reply = 'سرمایہ کاری: ڈپازٹ پیج پر جائیں، پلان منتخب کریں، Easypaisa/JazzCash سے ادائیگی کریں، اسکرین شاٹ اپ لوڈ کریں۔';
        else if (msg.includes('منافع') || msg.includes('پرافٹ')) reply = '60 دن تک 11% روزانہ منافع۔ مثال: PKR 360 = PKR 39.60 روزانہ × 60 = PKR 2,376';
        else if (msg.includes('نکاسی') || msg.includes('وڈرا')) reply = 'وڈرا: طریقہ منتخب کریں، اکاؤنٹ کی تفصیلات دیں، رقم درج کریں (کم از کم PKR 30)';
        else if (msg.includes('ریف') || msg.includes('دوست')) reply = 'ریف سسٹم: اپنا PID شیئر کریں، دوست کی سرمایہ کاری پر 11% بونس پائیں';
        else if (msg.includes('سلام') || msg.includes('ہیلو')) reply = 'السلام علیکم! پروفٹ 24 میں خوش آمدید۔ میں آپ کی کیا مدد کر سکتا ہوں؟';
        else reply = 'پروفٹ 24 کے بارے میں پوچھیں: سرمایہ کاری، منافع، نکاسی، ریفرل، یا ٹرانسفر';
    } else {
        if (msg.includes('invest') || msg.includes('deposit')) reply = 'To invest: Go to Deposit → Select plan → Pay via Easypaisa/JazzCash → Upload screenshot → Submit.';
        else if (msg.includes('profit')) reply = '11% daily profit for 60 days. Example: PKR 360 = PKR 39.60/day × 60 = PKR 2,376 total.';
        else if (msg.includes('withdraw')) reply = 'To withdraw: Select method → Enter account details → Enter amount (Min: PKR 30) → Submit.';
        else if (msg.includes('refer')) reply = 'Share your PID. Earn 11% when someone invests via your referral.';
        else if (msg.includes('hello') || msg.includes('hi')) reply = 'Hello! Welcome to Profit 24. How can I help you?';
        else reply = 'Ask about: Investment, Profit, Withdrawal, Referral, or Fund Transfer.';
    }
    res.json({ success: true, reply });
});

// ================================================================
// CRON - Daily Profit for ALL active plans
// ================================================================
    ...
    for (var i = 0; i < users.length; i++) {
        ...
        if (u.activePlans && u.activePlans.length > 0) {
            for (var j = 0; j < u.activePlans.length; j++) {
                var plan = u.activePlans[j];
                if (plan.endDate >= new Date() && plan.profitDays < 60) {
                    u.balance += plan.dailyProfit;
                    u.totalEarned += plan.dailyProfit;
                    plan.profitDays += 1;
                    await new Transaction({ userId: u._id, username: u.username, type: 'profit', amount: plan.dailyProfit, status: 'approved' }).save();
                }
            }
            await u.save();
        }
...
async function init() {
    ...
    if (await FAQ.countDocuments() === 0) await FAQ.insertMany([{ question: 'What is Profit 24?', answer: 'Secure investment platform with 11% daily profit for 60 days.', order: 1 }, { question: 'How to invest?', answer: 'Select plan → Pay via Easypaisa/JazzCash → Upload screenshot → Get approved.', order: 2 }, { question: 'How referral works?', answer: 'Share PID, earn 11% bonus when someone invests.', order: 3 }]);
}
